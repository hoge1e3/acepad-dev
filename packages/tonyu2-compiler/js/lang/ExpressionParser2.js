...
import { setRange } from "./parser.js";
//import Parser from "./parser";
const OPTYPE = Symbol("OPTYPE");
export function ExpressionParser(context, name = "Expression") {
    ...
    function composite(a) {
        ...
        return {
            ...
            get() {
                if (!e)
                    throw new Error("compsite: no parser added.");
                return e;
            }
        };
    }
    ...
    const $ = {
        ...
        lazy() {
            return context.create((st) => {
                const p = $.built;
                if (!p)
                    throw new Error("lazy: not built yet");
                return p.parse(st);
...
;
//# sourceMappingURL=ExpressionParser2.js.map