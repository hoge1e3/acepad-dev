...
import { isPostfix, isVarAccess, isNamedTypeExpr, isArrayTypeExpr, isUnionTypeExpr } from "./NodeTypes.js";
import { isNonArrowFuncInfo, isUnionType } from "./CompilerTypes.js";
...
const neverThrown = (n) => new Error("Never thrown this error");
...
function getSourceFile(klass) {
    ...
    if (!src)
        throw new Error("File for " + klass.fullName + " not found.");
    return src;
}
...
//-----------
export function initClassDecls(klass, env) {
    ...
    function initMethods(program) {
        ...
        if (t) {
            t.C.forEach(function (i) {
                ...
                if (!incc)
                    throw TError(R("classIsUndefined", n), s, p);
        ...
        klass.directives = {};
        /*if (!klass.superclass && klass.includes.length>0) {
            throw TError(R("includesCannotBeUsedWithoutSuperclass"), srcFile, program.incl?.pos||0,0);
        }*/
        //--
        ...
        var fieldsCollector = new Visitor({
            ...
            varsDecl(node) {
                if (ctx.inBlockScope && isBlockScopeDeclprefix(node.declPrefix))
                    return;
        ...
        program.stmts.forEach(function (stmt) {
            if (stmt.type == "funcDecl") {
                ...
                if (methods.hasOwnProperty(name))
                    throw TError(R("MethodAlreadyDeclared", name), srcFile, stmt.pos);
...
function annotateSource2(klass, env) {
    ...
    function initTopLevelScope() {
        ...
        if (!isTonyu1(env.options)) {
            for (let i in JSNATIVES) {
                s[i] = new SI.NATIVE("native::" + i, { class: globalThis[i], sampleValue: JSNATIVES[i] });
            }
        }
    ...
    function isFiberMethod(name) {
        if (stype(ctx.scope[name]) != ST.METHOD)
            return false;
    ...
    function prohibitGlobalNameOnBlockScopeDecl(v) {
        ...
        if (isg)
            throw TError(R("CannotUseGlobalVariableInLetOrConst"), srcFile, v.pos);
    }
    function getScopeInfo(node) {
        ...
        if (!t) {
            ...
            if (env.options.compiler.field_strict || klass.directives.field_strict) {
                if (!isg)
                    throw TError(R("fieldDeclarationRequired", n), srcFile, node.pos);
            }
            if (isg) {
    ...
    var localsCollector = new Visitor({
        ...
        varsDecl(node) {
            if (isBlockScopeDeclprefix(node.declPrefix))
                return;
            for (let d of node.decls) {
    ...
    var varAccessesAnnotator = new Visitor({
    ...
    function resolveType(node) {
        ...
        else if (isArrayTypeExpr(node))
            return resolveArrayType(node);
        else if (isUnionTypeExpr(node))
