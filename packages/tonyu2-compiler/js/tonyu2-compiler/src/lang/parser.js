...
export class Parser {
    ...
    and(next) {
        ...
        if (tbl[ALL]) {
            if (tbl[ALL].isEmpty &&
                ...
                if (next._first[ALL])
                    ntbl[ALL] = tbl[ALL].andNoUnify(next._first[ALL]);
            }
            else {
    ...
    ret(next) {
        ...
        if (tbl[ALL])
            ntbl[ALL] = tbl[ALL].retNoUnify(next);
        const res = this.context.fromFirst(ntbl);
        res.setAlias(_res);
        if (options.verboseFirst) {
...
*/
    ...
    unifyFirst(other) {
        ...
        function or(a, b) {
            ...
            if (!b && a)
                return a;
            if (!(a && b))
                throw new Error("One of them should present.");
    ...
    setAlias(p) {
        if (!p.name)
            throw new Error("setAlias: name is not set");
    ...
    setName(n, struct) {
        ...
        if (struct instanceof Parser) {
            if (!struct.name)
                throw new Error("setName: struct.name is not set");
...
export class State {
    ...
    _error;
    constructor(strOrTokens, global, cloneSrc) {
        if (strOrTokens != null) {
...
export const TokensParser = {
    ...
    token(type) {
        return tokensParserContext.create(function (s) {
            const src = s.src;
    ...
    eof: tokensParserContext.create((s) => {
        const src = s.src;
...
export function lazy(context, pf) {
    //let p:Parser;
    const lz = { resolve };
    function resolve() {
        if (!lz.resolved) {
