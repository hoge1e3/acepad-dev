...
//import { isTError } from "tonyu2-runtime/src/runtime/TError.js";
...
if (rename.do) {
    console.log(rename);
    builder.renameClassName(rename.from, rename.to).then(r => {
...
if (daemon)
    opt = { destinations: { file: false, memory: true } };
builder.fullCompile(opt).then(async function (s) {
    ...
    if (daemon) {
        ...
        prjDir.watch(async (e, f) => {
            ...
            if (now - lastRefreshed < 500)
                return;
            lastRefreshed = new Date().getTime();
            const ns = await builder.postChange(f);
            //console.log(ns);
            await ns.exec();
            if (globalThis.Tonyu.globals.$restart)
                globalThis.Tonyu.globals.$restart();
        });
    }
    if (run || daemon) {
