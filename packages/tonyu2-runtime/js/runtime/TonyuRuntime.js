...
import { isTonyuClass } from "./RuntimeTypes.js";
    ...
    define(params) {
        ...
        function extender(_parent, ctx) {
            ...
            if (includesRec[fullName]) {
                if (!parent)
                    throw TError(R("includesCannotBeUsedWithoutSuperclass"), params.fullName, 0, 0);
                return parent;
            }
        ...
        for (let nn in classes) {
            ...
            if (typeof nr === "function") {
                if (!found) {
                    ...
                    throw new Error(R("ambiguousClassName", nn, n, found));
            }
        }
    }
...
function invokeMethod(t, name, args, objName) {
    if (!t)
        throw new Error(R("cannotInvokeMethod", objName, t, name));
    var f = t[name];
    if (typeof f != "function")
        throw new Error(R("notAMethod", (objName == "this" ? "" : objName + "."), name, f));
    return f.apply(t, args);
}
function callFunc(f, args, fName) {
    if (typeof f != "function")
        throw new Error(R("notAFunction", fName));
    return f.apply({}, args);
}
function checkNonNull(v, name) {
    if (v != v || v == null)
        throw new Error(R("uninitialized", name, v));
    return v;
}
function A(args) {
    var res = [];
...
function run(bootClassName) {
    ...
    if (!isTonyuClass(bootClass))
        throw new Error(R("bootClassIsNotFound", bootClassName));
    Tonyu.runMode = true;
...
const Tonyu = {
    thread,
    supports_await: true,
    klass, bless, extend, messages: R, load, loadEvent: (e) => { },
