...
function isAceEditor(e){
  ...
  (e.commands && typeof e.setOptions==="function");
}
export function edit(e,opt){
    const ace=globalThis.ace;
    let editor = isAceEditor(e) ? e 
    :ace.edit(e);
    editor.setOptions({
...
//return ;
  //weird workaround
  //Unless this workaround, downward swipe reloads page.
  //even #editor-overlay sets pointer-events: none; , 
  //setting event handler has side-effect, that prevents "browser-default-behavior" 
...
export function selectKeyDevice(ps ,editor){
    ...
    const baseElement=document.querySelector(".acepad");
    if(ps.match(/^s/)){
      baseElement.classList.remove("phkey");
      editor.textInput.getElement().setAttribute("readonly",true);
      attachAcepadEditConmands(editor);
    }else {
      baseElement.classList.add("phkey");
