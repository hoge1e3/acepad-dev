...
export async function create({
    pass,ws,acepad,init,noalert,headless,
}){
  if(headless){
    return hl.create({pass,ws,init});
  }
    ...
    const telem=$("<div>").
      css(style).appendTo(wid.element);
    const eelem=$("<div>").
      css(style).appendTo(wid.element).hide();
    acepad.edit(eelem[0]);
    ...
    $("<button>").text("scroll").click(async()=>{
        w.send("\x01[");
    }).appendTo(btns).css({padding:"5px",background:"orange"});
    let te=0;
    $("<button>").text("editor").click(async()=>{
        //w.send("\x1b");
        te=!te;
        if(te){
          telem.hide();
    ...
    }).appendTo(btns).css({padding:"5px",background:"yellow"});
