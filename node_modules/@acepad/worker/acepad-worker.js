...
function header(){
    ...
    if (!purl || !purl.match(/^https:\/\//)) purl=cdn+"petit-node@latest/dist/index.js";
...
await pNode.boot({
    async init({FS}){
        try{
            ...
            // Not exists when nw-js
            if (FS.mountAsync) await FS.mountAsync("/idb/","idb");
            ...
            const cons=await pNode.importModule(FS.get(${em(consolePath)}));
            cons.client();
        }catch(e){
            console.error(e);
