...
import {init as initMode} from "./mode.js";
export {init as initMode} from "./mode.js";
/*global ace*/
...
export function createSession(info){
    ...
    ace.createEditSession(info.text||"");
    info.name=info.name||
      info.file&&info.file.name()||
      info.type&&`*${info.type}*`;
    info.type=info.type||"file";
    info.lastOpen=0;
    info.commands=command.parse(info.commands);
    sessionInfo(info.session,info);
    local.addCommandsToSession(info.session, info.commands);

    //data.session.$worker.send("setOptions",{esversion:8});
    // after changing the session
    info.session.on("changeMode", 
    (...a)=>setTimeout(()=>configureWorker(...a),500));
    //events.fire("createSession",info);
    sessionMap[info.name]=info.session;

    const conf=getConfig();
    applyConfig(conf.session||{}, info.session);
...
export function initSessions(editor){
    local.init(editor);
...
let getMainEditor,getConfig;//from './states.js';
    //events, // from './events.js';
    //locate,currentLine; // from './cursor.js';
export function extend(acepad) {
    getMainEditor=acepad.getMainEditor;
    getConfig=acepad.getConfig;
    //events=acepad.events;
    //currentLine=acepad.currentLine;

    return Object.assign(acepad,{
    });
}
