...
export function recset(){
  const avail=new Set();
  for(let r of recents.list()){
    //this.echo(r);
    r=file(r);
    while(r){
      avail.add(tsep(r.path()));
      r=r.up();
    }
  }
  return avail;
...
export function render(home,onsel){
  ...
  const element=t.div({
    class:"filetree-root",onclick},
      t.ul(tree(home))
    );
    setLevel(element);
    return element;
    function flash(felement){
      felement.classList.add("flash");
      felement.style.transition = "none";
      requestAnimationFrame(()=>{
        felement.style.transition = "opacity 0.4s";
        felement.style.opacity = "0.5";
          setTimeout(()=>{ 
            felement.style.transition=""; 
            felement.style.opacity=""; 
            felement.classList.remove("flash"); 
          }, 400);
      });
      ...
      function onclick(e){
        ...
        if(target.classList.contains("item")){
          const felement=element.closest(".widget")||element;
          ...
          if(f.isDir()){
            ...
            flash(felement);
            setTimeout(
              ()=>onsel(f),200);//sh.edit(f,{nomin:true});
          }
        }
        return true;
      }
