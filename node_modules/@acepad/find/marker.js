...
export function addMarker(
  ...
  highlight1.update = (
    ...
    if (options.update) options.update({
      html, markerLayer, session, config, range, options});
    const markerElement=getMarkerHTML(
      html, markerLayer, session, config, range, markerClass);
...
export function getMarkerHTML(html, markerLayer, session, config, range, markerClass) {
  ...
  if (range.isMultiLine()) {
    marked=markerLayer.drawTextMarker(
      stringBuilder, range, markerClass, config);
  } else {
    marked=markerLayer.drawSingleLineMarker(
      stringBuilder, range, `${markerClass} ace_start ace_br15`, config);
  }
...
export function initMarker(editor){
