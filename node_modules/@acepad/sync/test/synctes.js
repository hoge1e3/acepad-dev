...
export async function main(){
    ...
    putFiles(this.getcwd(),fix1);
    
    let id2=await commit(this.getcwd());
    this.echo("id2",id2);
    this.cd("../");
...
export async function deltes(name) {
    ...
    await commit(this.getcwd());
    this.cd("../ini/");
    await checkout(this.getcwd());
    this.rm(name);
    await commit(this.getcwd());
    this.cd("../clo/");
    await checkout(this.getcwd());
...
export async function fftes(){
    ...
    let id4=await commit(this.getcwd());
    this.echo("id4",id4);
    this.cd("../");
    ...
    let id5=await checkout(this.getcwd());
    this.echo("id5",id5);
    assert.equal(id4, id5, "id4!=id5");
    filesEq(this.getcwd(), newf);
    let id6=await commit(this.getcwd());
    this.echo("id6",id6);
    this.cd("../");
...
export async function inites(name){
    this.echo("init");
    let id=await init(name,this.getcwd());
...
export async function clonetes(name){
    this.echo("clone");
    let id=await clone(name, this.getcwd());   
    this.echo(id);
    let o=this.resolve(".sync/repo.json").obj();
    assert.equal(o.name,name);
    let o2=this.resolve(".sync/tree.json").obj();
