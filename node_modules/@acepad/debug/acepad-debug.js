...
import {currentLine,print,goLineEnd} from "@acepad/cursor";
...
export function debugSession(sh, cmd){
    ...
    const editor=sh.$acepad.getMainEditor();
    sh.setout({
        log(...a){
            dprint(...a);    
        }
    ...
    let dses=createSession({
        ...
        commands:{
            return(editor){
                onEnter({
                    editor,
                    line:currentLine(editor.session)
                });
            },
        },
    });
    ...
    if(!cmd){
      ...
      sh.evalCommand(cmd).then(r=>{
        if(r!=null)dprint(r);
        //console.log("result of",cmd,r);
      });
    }
    function hist(){
