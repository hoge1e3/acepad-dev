...
export async function main(p="immer"){
  //const installDir=findNodeModules.call(this);// path.join("/tmp/",p);
  //return installDir;
  await installSingle.call(this,p);
}
export async function installSingle(p){
  const _installDir=findNodeModules.call(this);// path.join("/tmp/",p);
  if (!_installDir) return "No installDir";
  const installDir=path.join(_installDir,p);
  console.log("installDir", installDir);
  const url=pkg2url(p);
  console.log(url);
  const j=await fetchJson(url);
  console.log(j);
  const {latest}=j["dist-tags"];
  const version=j.versions[latest];
  const tgz="/tmp/pkg.tgz";
  await download(
    version.dist.tarball,tgz);
  await extractTgz(tgz, installDir);
  await this.mv(
    ...this.glob(
      path.join(installDir,"package/*")),
    installDir
    );
  await this.rm({r:true},
  path.join(installDir,"package"));
}
export function findNodeModules(path) {
  for (let current = this.resolve(path); current; current = current.up()) {
    const nodeModules = current.rel("node_modules");
    if (nodeModules.exists()) {
      return nodeModules.path();
    }
  }
  return null;
   
}
export async function fromLock() {
