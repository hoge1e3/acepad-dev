...
import { modifierKeys } from "./types.js";
import { addModifierStyle, modifierLocked, getModifier, setModifier, clearUnlockedModifiers, modifierButtonPressed, modifierStateToKeyboardEvent, currentModifierState, } from './modifier.js'; //todo detect unused
...
function procChr(e) {
    ...
    if (syms[0].toLowerCase() !== syms[0].toUpperCase()) {
        ...
        $(e).empty().append($("<span>").addClass("mask").addClass("no-shift").text(syms[0])).append($("<span>").addClass("mask").addClass("shift").text(syms[1]));
        if (syms[2]) {
            $(e).find(".mask").addClass("no-sym");
            $(e).append($("<span>").addClass("mask").addClass("sym").text(syms[2]));
        }
    }
    else if (syms.length == 2) {
        $(e).empty().append($("<span>").addClass("mask").addClass("no-sym").text(syms[0])).append($("<span>").addClass("mask").addClass("sym").text(syms[1]));
    }
}
...
export function initKeypad({ events: _events }) {
    events = _events;
...
export function justify(row) {
    if (isHidden(row))
        return;
    let r = [...row.childNodes].filter((node) => isHTMLElement(node) &&
        !isHidden(node));
    let len = r.length;
...
export function getDatas(e, res = {}) {
    ...
    if (!res.command && !res.text &&
        !res.modifier && !res.keycode)
        res.text = e.innerText;
    return res;
}
...
export function showGuide(_b, size = 10) {
    let b;
    if (typeof _b === "string") {
        b = textToButton()[_b];
    }
    ...
    res[0].setAttribute("data-rect", JSON.stringify(r));
    ...
    function shrink() {
        ...
        size--;
        res[0].setAttribute("data-size", size);
        adjustSize(res[0]);
        if (size > 0)
