...
function readyPromise(document) {
  return new Promise((resolve) => {
    // Firefox bug
    if (document.readyState==="uninitialized") setTimeout(resolve,100);
...
function bypassConsole(w){
  const old={
    log:w.console.log,
    error:w.console.error,
  };
  w.console.log=(...a)=>console.log(...a);
  w.console.error=(...a)=>console.error(...a);
  
  w.addEventListener("unhandledrejection",(e)=>console.error(e.reason));
  w.addEventListener("error",(e)=>console.error(e.error));

}
export function available(){
  return !!document.querySelector("#acepad")||!!window.sh?.$acepad;
}
export function showIframe(url,options={}){
    ...
    let r=showWidget(cont, options);
    
    if(typeof url.text==="function"){
      (async()=>{
        const w=e[0].contentWindow;
        await readyPromise(w.document);
        bypassConsole(w);
        if(options.global){
          Object.assign(w,options.global);
        }
        await open(url,{
          globalThis: w,
          pNode: options.pNode,
...
export function showWidget(e="<div>",opt={}){
    ...
    document.querySelector("#editor-overlay")
    ...
    if(opt.overlay){
        overlay.activate(acepad,widall[0]/*e[0]*/,{dynamic:true});
        self.overlay=(input)=>overlay.overlay(acepad, input, {base: widall[0], dynamic:true});
    }
    return self;
}
function print(dom){
