#!run
import {Tonyu} from "tonyu2-runtime";
import {showWidget,showIframe,available} from "@acepad/widget";
import {open} from "@acepad/browser";
import {t} from "@hoge1e3/dom";
//import "./mode-tonyu.js";
//import "/idb/run/samples/tonyu/Actest/js/concat.js";
const raf=()=>new Promise(requestAnimationFrame); 
import {sibling} from "@acepad/here";
const html=sibling(import.meta.url,"html/");
export async function main(...a){
  const [opt,...args]=this.pickOptions(a);
  if (!opt.nc) {
    await this.tonyuc();
  }
  let g;
  if (available()) {
    const w=showIframe(html.rel("index.html"),opt);
    g=w.iframe.contentWindow;
  } else {
    await open(html.rel("index.html"));
    g=globalThis;
  }
  while(!g.main)await this.sleep(0.01);
  g.main(this.resolve(".").path());
}
export async function init() {
  let cv=t.canvas();
  let wid=showWidget();
  const e=wid.element;
  const rect=e.getBoundingClientRect();
  cv.setAttribute("width",rect.width);
  cv.setAttribute("height",rect.height);
  wid.element.appendChild(cv);
  let g=Tonyu.globals;
  const sh=this;
  //sh.cd("/idb/run/samples/tonyu/Actest/");
  await sh.import(".");
  
  g.$mainCanvas=$(cv);
  g.$shell=this;
  g.$pNode=pNode;
  globalThis.UIDiag={};
  g.$currentProject={
    isAcepad:true,
      getResource(){
        const r=this.getDir().rel("res.json").obj();
        /*const r={
          images:[],sounds:[]
        };*/
        console.log("getResource",r);
        return r;
      },
      getOptions:()=>({
          run: {
              mainClass: "user.Main",
          },
      }),
      loadPlugins:(s)=>s(),
      requestPlugin:(s)=>console.log("Plugin", s),
      getDir() {
        return sh.resolve(".");
      }
  };
  /*globalThis.ImageList={
      load:(_,succ)=>succ({names:[]})
  };*/
  globalThis.T2MediaLib=class {
    activate(){
    }
  };
  Tonyu.run("kernel.Boot");
  return ;
}

