...
import { asFilename, asHash, asMode, Author, isObjectType, asBranchName, asLocalRef, asFilePath, asPathInRepo } from './types.js';
...
export class Repo {
    ...
    constructor(gitDir, objectStore) {
        this.gitDir = gitDir;
    ...
    toFilePath(pathInRepo) {
        const r = path.join(this.workingDir(), pathInRepo);
        return asFilePath(r);
    }
    toPathInRepo(filePath) {
        const r = path.relative(this.workingDir(), filePath);
        if (r.startsWith(".."))
            throw new Error(`${filePath} is out of working dir ${this.workingDir()}`);
    ...
    static async objectEntryToGitObject(objval) {
    ...
    static async gitObjectToTree({ type, content }) {
        if (type !== 'tree') {
    ...
    async setHashToFMT(path, hash) {
        const fms = await this.getFMTStorage();
        if (!fms)
            return;
        const stat = await fs.lstat(path);
        if (!stat.hasFineMtime) {
            return;
        }
        const mtime = stat.mtimeMs;
        await fms.set(path, { hash, mtime });
    }
    async getHashFromFMT(path) {
        const fms = await this.getFMTStorage();
        if (!fms)
            return null;
        const stat = await fs.lstat(path);
    ...
    static async gitObjectToCommitEntry({ type, content }) {
        if (type !== 'commit') {
        ...
        if (!committer)
            throw new Error("Missing commiter");
        return {
    ...
    async *traverseTree(entries, prefix = '') {
        for (const entry of entries) {
    ...
    async diffTreeRecursive(oldTree, newTree, prefix = '') {
        const diffs = [];
    ...
    async threeWayMerge(baseTree, aTree, bTree) {
    ...
    async checkoutTreeToDir(treeHash, dirPath) {
    ...
    async setCurrentBranchName(branch) {
        const headPath = this.headPath();
    ...
    async writeMergeHead(commitHash) {
        const MERGE_HEAD = join(this.gitDir, "MERGE_HEAD");
        if (commitHash) {
    ...
    realGitRepoIsSubRepo() {
        return false; // TODO: configure
    }
    async isSubRepo(dir) {
        if (!this.inWorkingDir(dir))
            return false;
        const dot_gsync = path.basename(this.gitDir);
        return (this.realGitRepoIsSubRepo() && await exists(join(dir, ".git"))) ||
            await exists(join(dir, dot_gsync));
    }
    async inSubRepo(_path) {
        if (!this.inWorkingDir(_path))
            return false;
        const workDir = this.workingDir();
        for (; path.normalize(workDir) !== path.normalize(_path); _path = asFilePath(path.dirname(_path))) {
            if (await this.isSubRepo(_path))
                return true;
        }
        return false;
    }
    async applyDiff(diffs) {
        const workDir = this.workingDir();
        const igc = await IgnoreChecker.init(this);
        for (const diff of diffs) {
            ...
            if (await igc.ignores(filePath))
                continue;
            if (await this.inSubRepo(asFilePath(path.dirname(filePath))))
                continue;
            if (diff.type === 'deleted') {
                ...
                if (!diff.newHash)
                    throw new Error(`Missing 'other' hash for ${diff.path}`);
                const { type, content } = await this.readObject(diff.newHash);
                if (type !== 'blob')
                    throw new Error(`Expected blob, got ${type} for ${diff.path}`);
    ...
    async hasSymlinkInPath(targetPath) {
        ...
        for (const part of parts) {
            if (!part)
                continue;
            if (part == "..")
                throw new Error(`${rel}(${targetPath}) is out of this repo.`);
    ...
    async updateIndexFromWorkingDir(indexPath) {
        const workingDir = this.workingDir();
    ...
    async writeTreeFromIndex(index) {
        ...
        const writeNodeTree = async (node) => {
            const entries = [];
...
export async function writeFileIgnoreingCRLF(filePath, content) {
    if (await exists(filePath)) {
        const org = await fs.readFile(filePath);
        if (sameExceptCRLF(org, content))
            return;
    }
...
export class IgnoreChecker {
    ...
    async get(dir) {
        if (!this.repo.inWorkingDir(dir))
            throw new Error(`${dir} is out of working dir ${this.repo.workingDir()}`);
        if (this.igs.has(dir)) {
...
export function stripCR(content) {
...
export function sameExceptCRLF(a, b) {
    const sa = stripCR(a);
    const sb = stripCR(b);
    if (sa.byteLength !== sb.byteLength)
        return false;
    for (let i = 0; i < sa.byteLength; i++)
        if (sa[i] !== sb[i])
            return false;
    return true;
}
