...
export function isRelPath(s) {
    return true;
}
const SymAbsPath = Symbol("path");
export function isAbsPath(s) {
    return true;
}
export function isPath(s) {
    return isAbsPath(s) || isRelPath(s);
}
export function asPath(s) {
    if (!isPath(s))
        throw new Error(`${s} is not a path`);
    return s;
}
export function asRelPath(s) {
    if (!isRelPath(s))
        throw new Error(`${s} is not a relative path`);
    return s;
}
