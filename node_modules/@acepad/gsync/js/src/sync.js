...
import { asHash, asFilePath } from './types.js';
...
export class Sync {
    ...
    async readState() {
        ...
        const state = JSON.parse(await fs.promises.readFile(statefile, { encoding: "utf-8" }));
        return state;
    }
    stateFile() {
    ...
    async uploadObjects() {
        ...
        for (const dir of dirs) {
            ...
            for (const file of files) {
                ...
                if (stats.mtimeMs < state.uploadSince * 1000)
                    continue; // 秒→ミリ秒で比較
                const hash = dir + file;
