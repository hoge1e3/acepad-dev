    ...
    try {
        ...
        switch (command) {
            ...
            case "share":
                return share(cwd);
            case "scan":
                return await scan(cwd, args.includes("--id"), args.includes("--url"), args.includes("--key"), args.includes("--shell"));
            case "switch":
            case "checkout":
                {
                    if (args.length === 0) {
...
async function resetHard(cwd, targetBranch) {
...
export async function scan(cwd, showRepo, showUrl, showKey, shell) {
...
export async function share(cwd, gitDirName = GIT_DIR_NAME) {
...
export async function manage(cwd, gitDirName = GIT_DIR_NAME) {
    const dir = path.join(cwd, gitDirName);
    const gitDir = asFilePath(dir);
    const syncf = new SyncFactory(gitDir);
    const conf = await syncf.readConfig();
...
export async function catFile(dir, hash) {
...
export async function init(cwd, serverUrl, gitDirName = GIT_DIR_NAME) {
    const dir = path.join(cwd, gitDirName);
...
export async function clone(into, serverUrl, repoId, branch = "main", options = { gitDirName: GIT_DIR_NAME }) {
    return await _clone(asFilePath(into), { serverUrl, repoId, apiKey: Math.random().toString(36).slice(2) }, asBranchName(branch), options);
}
async function _clone(into, config, branch, options) {
    let skipco;
...
export async function findGitDir(cwd) {
    let c = cwd;
    while (true) {
        ...
        if (!nc || nc === c)
            throw new Error(`No git repo found from ${cwd}`);
        c = nc;
    }
}
let verbose = false;
...
export async function syncWithRetry(dir, conflictResolutionPolicy) {
    const res = [];
    for (let i = 0; i < 5; i++) {
...
export async function sync(dir, conflictResolutionPolicy = "saveHashedRemote", message = new Date() + "") {
        ...
        for (let c of conflicts) {
            ...
            if (!sameExceptCRLF(localContent, remoteObj.content)) {
                ...
                if (winner === null) {
                    ...
                    const [merged, hasConflict] = baseContent_str && localContent_str && remoteContent_str ?
...
async function conflictedFile(repo, filePath, postfix) {
    ...
    const dst = makePostfix(path.join(work, GSYNC_CONFLICT_DIR, rel), postfix);
    return dst;
}
function makePostfix(filepath, postfix) {
...
export async function log(dir, check_ref = false) {
    const gitDir = await findGitDir(asFilePath(dir));
    const repo = await offlineRepo(gitDir);
    ...
    while (c) {
        ...
        if (check_ref)
            console.log("Scanned objects: ", await checkRef(repo, ch));
        let next;
...
export async function checkRef(repo, chash) {
...
export async function cmdListBranches(dir) {
    const gitDir = await findGitDir(asFilePath(dir));
    const repo = await offlineRepo(gitDir);
...
export async function switchBranch(dir, branchName, options = {}) {
    const gitDir = await findGitDir(asFilePath(dir));
    const syncf = new SyncFactory(gitDir);
    const sync = await syncf.load();
    const repo = sync.repo;
...
export async function mergeBranch(dir, sourceBranchName) {
    const gitDir = await findGitDir(asFilePath(dir));
    const syncf = new SyncFactory(gitDir);
    const sync = await syncf.load();
    const repo = sync.repo;
...
export async function diffCmd(dir, args) {
