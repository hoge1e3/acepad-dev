...
import { GIT_DIR_NAME, SyncFactory } from "./sync.js";
import { asBranchName, asFilePath, asHash, asLocalRef, Author, isHash } from "./types.js";
...
export async function main(cwd = process.cwd(), argv = process.argv, shell = new NodeShell()) {
    ...
    try {
        ...
        switch (command) {
            ...
            case "share":
                return cli.share();
            case "scan":
                return await cli.scan(args.includes("--id"), args.includes("--url"), args.includes("--key"), args.includes("--shell"));
            case "switch":
            case "checkout":
                {
                    if (args.length === 0) {
...
export class Cli {
    ...
    shell;
    verbose = false;
    constructor(cwd = process.cwd(), shell = new NodeShell()) {
        this.cwd = cwd;
        this.shell = shell;
    }
    async echo(...a) {
    ...
    async resetHard(targetBranch) {
        ...
        if (!remoteHead)
            throw new Error(`No remote HEAD for branch '${branch}'`);
    ...
    async scan(showRepo, showUrl, showKey, shell) {
        ...
        for (let d of await scanDir(asFilePath(this.cwd))) {
            ...
            if (showKey || showRepo || showUrl) {
                ...
                if (showUrl)
                    field.push(conf.serverUrl);
                if (showRepo)
                    field.push(conf.repoId);
                if (showKey)
                    field.push(conf.apiKey);
            }
            await this.echo(...field);
        }
    }
    async share(gitDirName = GIT_DIR_NAME) {
    ...
    async manage(gitDirName = GIT_DIR_NAME) {
        const dir = path.join(this.cwd, gitDirName);
        const gitDir = asFilePath(dir);
        const syncf = new SyncFactory(gitDir);
        const conf = await syncf.readConfig();
    ...
    async init(serverUrl, gitDirName = GIT_DIR_NAME) {
        const dir = path.join(this.cwd, gitDirName);
    ...
    async clone(into, serverUrl, repoId, branch = "main", options = { gitDirName: GIT_DIR_NAME }) {
    ...
    async _clone(into, config, branch, options) {
        let skipco;
    ...
    async findGitDir() {
        ...
        while (true) {
            ...
            if (!nc || nc === c)
                throw new Error(`No git repo found from ${this.cwd}`);
            c = nc;
        }
    }
    ...
    async syncWithRetry(conflictResolutionPolicy) {
        const res = [];
        for (let i = 0; i < 5; i++) {
    ...
    async sync(conflictResolutionPolicy = "saveHashedRemote", message = new Date() + "") {
        ...
        if (conflicts.length == 0) {
            ...
            for (let c of conflicts) {
                ...
                if (postfixedPath) {
                    ...
                    if (confpaths.length == 1)
                        await this.echo("CONFLICT");
                }
            }
    ...
    async conflictedFile(repo, filePath, postfix) {
        ...
        if (rel.startsWith(".."))
            throw new Error(`${filePath} is out of ${work}`);
        const dst = Cli.makePostfix(path.join(work, GSYNC_CONFLICT_DIR, rel), postfix);
        return dst;
    }
    async processConflict(repo, c, commitHash, conflictResolutionPolicy, remoteCommitTime) {
        ...
        if (sameExceptCRLF(localContent, sourceObj.content))
            return null;
        const winner = conflictResolutionPolicy === "ignoreLocal" ? "remote" :
        ...
        const [merged, hasConflict] = baseContent_str && localContent_str && sourceContent_str ?
    ...
    static makePostfix(filepath, postfix) {
    ...
    async log(check_ref = false) {
        ...
        while (c) {
            ...
            if (check_ref)
                await this.echo("Scanned objects: ", await Cli.checkRef(repo, ch));
            let next;
    ...
    async mergeBranch(sourceBranchName) {
        ...
        let sourceCommitHash = localSourceCommitHash || remoteSourceCommitHash;
        if (!sourceCommitHash) {
    ...
    async showLineDiff(path, oldText, newText) {
    ...
    async showLines(path, text, prefix) {
...
export function scan(cwd, showRepo, showUrl, showKey, shell) {
    return new Cli(cwd).scan(showRepo, showUrl, showKey, shell);
}
export function share(cwd, gitDirName = GIT_DIR_NAME) {
    return new Cli(cwd).share(gitDirName);
}
export function manage(cwd, gitDirName = GIT_DIR_NAME) {
    return new Cli(cwd).manage(gitDirName);
}
export function msync(dir, branchName) {
    return new Cli(dir).msync(branchName);
}
export function catFile(dir, hash) {
    return new Cli(dir).catFile(hash);
}
export function init(cwd, serverUrl, gitDirName = GIT_DIR_NAME) {
    return new Cli(cwd).init(serverUrl, gitDirName);
}
export function clone(into, serverUrl, repoId, branch, options) {
...
export function syncWithRetry(dir, conflictResolutionPolicy) {
    return new Cli(dir).syncWithRetry(conflictResolutionPolicy);
}
export function sync(dir, conflictResolutionPolicy, message) {
    return new Cli(dir).sync(conflictResolutionPolicy, message);
}
export function log(dir, check_ref = false) {
    return new Cli(dir).log(check_ref);
}
export function checkRef(repo, chash) {
    return Cli.checkRef(repo, chash);
}
export function cmdListBranches(dir) {
    return new Cli(dir).listBranches();
}
export function switchBranch(dir, branchName, options = {}) {
    return new Cli(dir).switchBranch(branchName, options);
}
export function mergeBranch(dir, sourceBranchName) {
    return new Cli(dir).mergeBranch(sourceBranchName);
}
export function diffCmd(dir, args) {
