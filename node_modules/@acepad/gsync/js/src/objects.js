import { asFilePath, asHash, asPHPTimestamp } from "./types.js";
...
async function mtimeOnPut(store, downloaded) {
...
export async function factory(gitDir, repoId) {
    ...
    if (globalThis.indexedDB && !await exists(objdir)) {
        if (!repoId) {
            ...
            const conf = JSON.parse(await fs.readFile(conffile, { encoding: "utf-8" }));
            repoId = conf.repoId;
        }
...
export async function maxMtime(o) {
    let max = new Date(0);
...
export function singleStoreTransaction(db, table, mode = "readwrite") {
...
export class IndexedDBBasedObjectStore {
        ...
        request.onsuccess = async (event) => {
            ...
            if (await reqP(countRequest) === 0) {
                ...
                if (await exists(this.stateFile)) {
                    const state = JSON.parse(await fs.readFile(this.stateFile, { encoding: "utf-8" }));
    ...
    async put(hash, compressed, downloaded) {
        await this.dbInit;
        const mtime = await mtimeOnPut(this, downloaded);
        return new Promise((resolve, reject) => {
            const transaction = this.db.transaction(STORE_NAME, "readwrite");
            const value = { content: compressed, mtime };
            const store = transaction.objectStore(STORE_NAME);
