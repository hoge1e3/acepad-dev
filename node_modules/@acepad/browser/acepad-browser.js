...
export async function open(f,_opt={}){
    ...
    if(_opt.pNode){
      ...
      opt.toURL=async (p,n)=>{
        let t=n.getAttribute("type");
        if(t=="module"){
            return await pn.createModuleURL(opt.resolve(p));
        }
        return await pn.createURL(opt.resolve(p));
      };*/
      return ;
    }
...
function pNodeHeader(){
  ...
  function header(){
    ...
    return `const pNode=await import(${JSON.stringify(purl)});`;
  }
