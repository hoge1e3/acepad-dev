...
import {createModuleURL, getCore, default as pNode} from "petit-node";
...
export async function open(f,_opt={}){
    ...
    if(_opt.pNode!==false && opt.globalThis!==globalThis){
      const g=opt.globalThis;
      const pn=pNode.clone(g);//await loadPNode(g, getCore());
      await pn.boot({core:pNode.core});
      g.process=g.process||process;//TODO: process.__clone(g);
