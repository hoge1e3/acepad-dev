// acepad-overlay
import * as _overlay from "ace-overlay";
import {bounded} from "@hoge1e3/dom-bind";

export function option(acepad, opt={}){
    return {
        oncreate({editor}){
            acepad.edit(editor);
...
export function overlay(acepad, input, opt={}) {
  return _overlay.overlay(input,option(acepad,opt));
}
export function activate(acepad,container,opt={}){
    _overlay.activate(container,option(acepad,opt));/*{
        oncreate({editor}){
            acepad.edit(editor);
            acepad.setCurrentEditor(editor);
        },
