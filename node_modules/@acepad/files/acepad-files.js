// acepad-files
//import timer from "pause-me";
//assert
import MutablePromise from "mutable-promise";
import {ok} from "assert";
import {match} from "textmatcher";
import {initMode} from "@acepad/mode-shell";
import {convertStack} from "@acepad/os";
import {hasTrace} from "@acepad/debug";
import * as recents from "./recents.js";
export * as recents from "./recents.js";
import * as pos from "./pos.js";
import {shd, init as initAutoSync} from "./autoSync.js";
import {sibling,file} from "@acepad/here";
import pNode from "petit-node";
import {sessionInfo, createSession, 
recentInfos,findSession, 
removeSession,
  initMode as initSessionMode
} from "@acepad/sessions";
import {locate,currentLine,print} from "@acepad/cursor";
import * as path from "path";
import {retry} from "@acepad/retry";
import * as mode from "@acepad/mode";
import "./mode-tonyu.js";
let an;
let modeset;
/*const an=await pNode.import(sibling(
  import.meta.url,"annotations.js"));*/
export async function setAnnotations(...a){
  an=an||await pNode.import(sibling(
  import.meta.url,"annotations.js"));
  return an.setAnnotations(...a);
}
//function stripPrompt(line){
  //return line.replace(/^sh:\s*/,"");
//}
export function current(sh){
  const acepad=sh.$acepad;
  const editor=acepad.getMainEditor();
  const session=editor.session;
  const si=sessionInfo(session);
  return si.file;
}
/*global ace*/
export async function openFile(sh,fn,opt){
  const acepad=sh.$acepad;
  const {getMainEditor}=acepad;
  const editor=getMainEditor();
  initAutoSync(editor);
  pos.initAutoSave(acepad);
  let f;
  const ns=(v)=>typeof v==="number"||typeof v==="string";
  const move=()=>{
    console.log("loc",opt);
    if(opt&&
    ns(opt.row)&&ns(opt.column)
    )editor.gotoLine(opt.row-0,opt.column-0);
  };
  await retry(()=>{
    if(typeof fn=="string"){
      f=sh.resolve(fn).resolveLink();
    }else{
      f=fn.resolveLink();
      fn=f.name();
    }
  });
  try {
    //if(!f.isDir())
    recents.add(sh, f);
  }catch(e){
    console.error(e);
    alert(e);
  }
  let s=findSessionByFile(f);
  if(s){
    editor.setSession(s);
    move();
    return s;
  }
  if(f.isDir()){
    s=createDirList(sh,f);
    editor.setSession(s);
    return s;
  }
  if(!f.exists())f.text("");
  let text=f.text();
  //let t;
  s=createSession({
    text,
    name:`${f.name()}(${f.up()||""})`,
    file:f,
    type:"file",
    ts:f.lastUpdate(),
    val:text,
    onActivated(){
    },
    onDeactivated(){
      shd("pause",f);
      //t.pause();
    },
    commands:{
      F7(){
        openFile(sh, f.up());
      },
    }
  });
  let si=sessionInfo(s);
  si.on("activate",({oldSession})=>{
    if(!f.exists()){
      removeSession(s);
      let o=findSession(oldSession);
      if(o)setTimeout(()=>
      editor.setSession(o),100);
      return ;
    }   
    shd("resume",f);
  });
  editor.setSession(s);
  if(!opt){
    const {row,column}=pos.get(f);
    editor.gotoLine(row-0,column-0);
  }
  const mode=modeMap[f.ext()];
  if(mode){
    s.setMode(mode);
  }else if(f.text().match(/^(\/\/|#)!run/)){
    s.setMode("ace/mode/javascript");
  }
  move();
  return s;
}
export const modeMap={
  ".html":"ace/mode/html",
  ".php":"ace/mode/php",
  ".c":"ace/mode/c",
  ".py":"ace/mode/python",
  ".json":"ace/mode/json",
  ".css":"ace/mode/css",
  ".js":"ace/mode/javascript",
  ".cjs":"ace/mode/javascript",
  ".mjs":"ace/mode/javascript",
  ".ts":"ace/mode/typescript",
  ".tonyu":"ace/mode/tonyu",
  //".tonyu":"ace/mode/javascript",
  ".":"ace/mode/",
};
const unh=Symbol("unhacked");
function cdhacked(_sh,dir){
  const unhacked=_sh[unh]||_sh;
  const sh=_sh.clone();
  sh.cd(dir);
  const hacked=sh;
  hacked[unh]=unhacked;
  hacked.cd=function (dir){
    openFile(this,this.resolve(dir));  
  };
  hacked.clone=function (){
    const nsh=unhacked.clone.call(this);
    nsh[unh]=null;
    nsh.cd=unhacked.cd;
    nsh.clone=unhacked.clone;
    return nsh;
  };
  return sh;
}
export function createDirList(sh,dir){
  const acepad=sh.$acepad;
  const resolve=(n)=>sh.resolve(n);//dir.rel(n);
  sh=cdhacked(sh,dir);
  /*sh=sh.clone();
  sh.cd(dir);*/
  sh.setout({
    log(...a){
      print(s,...a);    
    }
  });
  let editor, reqReload=true;
  function reload(editor) {
    reqReload=true;
    onActivated(editor);
  }
  
  async function onActivated(_editor){
    editor=_editor;
    if (!reqReload) return;
    reqReload=false;
    ok(editor.session,"why session undefined");
    editor.session.setValue(
    dir+"📁\n"+
    (await retry(
    ()=>dir.listFiles())).
    sort((a,b)=>
      b.lastUpdate()-
      a.lastUpdate()
    ).map((p)=>p.name())
    .join("\n")+            
    "\n../"+
    "\nnew: "+
    "\nctrl-t: rename ctrl-e: remove\n"+
    //dir+
    "sh: \n"+sh.history().map(s=>`sh: ${s}`).join("\n"));
    locate(editor.session,0,1);
  }
    // todo: same as @acepad/debug
    let inpmode;
    sh.input=function (prompt="? "){
        this.echo(prompt,{n:1});
        inpmode=new MutablePromise();
        return inpmode.then((r)=>{
            this.echo("");
            const p=prompt.trim();
            if (r.startsWith(p)) r=r.substring(p.length).trim();
            return r;
        });
    };

  let s=createSession({
    type:"files",
    file:dir,
    sh,
   name:`${dir.name()}/ (${dir.up()||""})`,
    
   // name:dir.name()+"/",
    onActivated,
    commands:{
      "ctrl-s":function (){
        locate(s,0,null);
        print(s,"sh: ");
      },
      "ctrl-e":function (){
        locate(s,0,null);
        print(s,"rm: ");
      },
      "ctrl-t":function (){
        const fn=currentLine(s);
        locate(s,0,null);
        print(s,"mv: ");
        locate(s,100,null);
        print(s," to: "+fn);
      },
      F6(){
        reload(editor);
      },
      F7(){
        openFile(sh, dir.up());
      },
      return(){
        retry(onEnter);
      }, 
    },
  });
  try{
  initMode();
  s.setMode("ace/mode/shell");
  }catch(e){
    console.error(e);
  }
  async function onEnter(e){
    let rm,mv;
    let f;
    let pos;
    if(inpmode){
      inpmode.resolve(currentLine(s));
      inpmode=null;
      return ;
    }
    const ex=(c)=>{
      locate(s, -1);
      print(s,"\n");
      sh.addHist(c);
      Promise.resolve(sh.exec(c)).then(
        (r)=>sh.echo(r),
        (e)=>{
          e=convertStack(e);
          print(s,e.stack||e);
        });   
    };
    match(currentLine(s),
      /^new: *(\S+)/,
      (_,_0,n)=>{
        f=resolve(n);
        if(!f.exists()) {
          if (n.match(/\/$/)) {
            f.mkdir();
          } else {
            f.text("");
          }
        }
        return n;
      },
      /^rm:? *(.+)/,
      (_,_0,n)=>{
        f=resolve(n);
        rm=true;
        return n;
      },
      /^mv:? *(\S+) +to: +(\S+)/,
      (_,_0,n,t)=>{
        f=resolve(n);
        mv=resolve(t);
        return n;
      },
      /^sh: *(.+)/,
      (_,_o,c)=>ex(c), 
      (line)=>{
        const tr=hasTrace(line);
        if(tr){
          f=tr.file;
          pos=tr;
        }else{
          f=resolve(line);
        }
      });
    if(!f)return ;
    if(!f.exists()){
      ex(currentLine(s));
      return ;
    }
    if(rm){
      if(confirm("remove "+f.name()+"?")){
        f.rm();
        reload(editor);
      }
    }else if(mv){
      if(mv.exists()){
        alert(`${mv.name()} exists`);
      }else{
        f.moveTo(mv);
        reload(editor);
      }
    }else{
      await openFile(sh,f,pos);
    }
  }
  return s;
}
export function findSessionByFile(f){
  return findSession((s,si)=>si.file&&
  si.file.equals(f));
}
export function createSessionListWithRecents(sh){
  const acepad=sh.$acepad;
  const editor=acepad.getMainEditor();
  removeSession("*sessions*");
  let rf;
  const s=createSession({
    type:"sessions",
    name:"*sessionsF*",
    onActivated(editor){
      const opened=new Set();
      let sec=0,recc=0;
      s.setValue("");
      for(let si of recentInfos()){
        if(si.session===s)continue;
        print(s,si.name+"\n");
        sec++;
        if(si.file)opened.add(si.file.path());
        rf=rf||si.file;
      }
      if(sec){
        locate(s,0,0);
        print(s,"[Session List]\n");
        locate(s,0,-1);
      }
      const {row}=locate.get(s);
      //print(s,"[Recents]\n");
      for(let f of recents.list(sh)){
        rf=rf||f;
        try{
          const ff=file(f);
          if(ff.isDir())continue;
          if(!ff.exists())continue;
        }catch(e){
          continue;          
        }
        if(opened.has(f))continue;
        print(s,revp(f)+(
          /*f.isDir()?"/":*/""
          )+"\n");
        recc++;
      }
      if(recc){
        locate(s,0,row);
        print(s,"[Recents]\n");
        locate(s,0,-1);
      }
    
      const home=sh.$home;
      
      if(home && !opened.has(home)){
        print(s,home+"\n");
      }
      locate(s,0,2);
    },
    commands: {
      async return() {
        const line=currentLine(s);
        const ns=findSession(line);
        if(ns){
          editor.setSession(ns);
        }else{
          const f=await resrevp(sh,line);
          openFile(sh,f);
        }
      },
      F7:()=>{
        openFile(sh,rf);
      },
    },
  });
  initSessionMode();
  s.setMode("ace/mode/sessions");
  return s;
}
function revp(p){
  return `${path.basename(p)+(
    p.endsWith("/")?"/":""
    )}\t${path.dirname(p)}`;
}
async function resrevp(sh,line){
  return await retry(()=>{
    try{
      return sh.resolve(line,true);
    }catch(e) {
      const [n,d]=line.split("\t");
      const np=path.join(d,n);
      return sh.resolve(np,true);
    }
  });
}