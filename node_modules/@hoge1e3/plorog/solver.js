...
export function *solve(db,predicate,outer){
    ...
    for(let rule of db){
        ...
        const ctx2=match(predicate, rule.head, ctx);
        //   ctx2  {Y:^X}
        if(!ctx2)continue;
        if(yield* solveConds(db, rule.conds, ctx2)) break; 
    }
    solve.depth--;
}
solve.depth=0;
export function* solveConds(db, conds,ctx){
    ...
    for(let outer of solve(db, car, ctx.outer)){
        ...
        if(yield* solveConds(db, cdr, ctx2))return true;
    }
    return false;
}
export function showAns(res){
    if(res.size==0){
        console.log("yes");
    }
    for(let [k,v] of res){
