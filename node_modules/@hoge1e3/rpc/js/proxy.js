import { Client } from "./client.js";
import { Server } from "./server.js";
import { isMessegable } from "./types.js";
export function server(...a) {
    const target = (isMessegable(a[0]) ?
        a.shift() :
        globalThis);
    const channel = a.shift();
    const allowOrigin = a.shift();
    const methods = a.shift();
    const s = new Server(target, channel, allowOrigin);
    for (let k in methods) {
        s.serv(k, (params) => methods[k](...params.args));
    }
    return s;
}
export function client(target, channel = "default", origin = undefined, manualProbe = false) {
    const c = new Client(target, channel, origin, manualProbe);
    return new Proxy(c, {
        get(target, p) {
            if (p === "__unproxy__") {
                return c;
            }
            if (typeof p === "symbol" || p === "then" || p === "toString") {
                return Reflect.get(target, p);
            }
            //const t=args.map((a:any)=>transfers.get(a))
            return (...args) => {
                //const t = args.reduce((tr, a) => { var _a; return [...t, ...(_a = transfers.get(a)) !== null && _a !== void 0 ? _a : []]; });
                return c.run(p, { args });
            };
        }
    });
}
const transfers = new WeakMap();
export function transfer(o, t) {
    if (!t)
        t = [o];
    transfers.set(o, t);
    return o;
}
function isWorkerPeerParam(p) {
    return p.target instanceof Worker;
}
export function peer(param) {
    var _a;
    const [targetOrigin, allowOrigin] = isWorkerPeerParam(param) ?
        [undefined, []] :
        [param.targetOrigin, (_a = param.allowOrigin) !== null && _a !== void 0 ? _a : [param.targetOrigin]];
    const { target, service, channel } = param;
    const cli = client(target, channel, targetOrigin, false);
    const srv = server(globalThis, channel, allowOrigin, service);
    cli.__unproxy__.addEventListener("dispose", () => srv.dispose());
    return cli;
}
//# sourceMappingURL=proxy.js.map