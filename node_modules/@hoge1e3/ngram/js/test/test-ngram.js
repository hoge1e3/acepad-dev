...
import { Document, DocumentSet, DocumentIndex, LetterIndex } from "../src/ngram.js";
import * as assert from "assert";
function echo(...args) {
    console.log(...args);
}
function collectWords(words, content) {
    // Split the content into words using a regex to handle different delimiters (e.g., spaces, punctuation).
    const extractedWords = content.split(/\W+/);
    // Iterate through the extracted words.
    for (const word of extractedWords) {
        // Add non-empty words to the set.
        if (word.trim()) {
            words.add(word.trim().toLowerCase()); // Convert to lowercase to avoid duplicates.
        }
    }
}
function getFixture(dir, ds) {
    const s = new Set();
    for (let f of dir.recursive()) {
        if (f.ext() !== ".js")
            continue;
        const d = new Document(f.path(), f.text());
        ds.addDocument(d);
        collectWords(s, d.content);
    }
    return s;
}
export function main() {
    return __awaiter(this, void 0, void 0, function* () {
        let fixture = this.resolve("node_modules");
        //let d=new Document("A","abcc deff adc eedef abc");
        let s = new DocumentSet();
        const words = getFixture(fixture, s);
        //s.addDocument(d);
        let scale = "";
...
function find(ds, word) {
    ...
    for (let { document, offset } of ds.find(word)) {
        ...
        assert.ok(document.looks(offset, word), [word, document.path, offset].join(","));
    ...
    for (let [p, d] of ds.path2doc) {
        while (true) {
            ...
            if (i < 0)
                return;
            offset = i;
            assert.ok(r.has(`${d}:${offset}`), `no ${d}:${offset}`);
            offset++;
            found++;
        }
    }
    ...
    return __awaiter(this, void 0, void 0, function* () {
        for (let [p, d] of ds.path2doc) {
            for (let i = 1; i < d.content.length; i++) {
                ...
                for (let [k, v] of yield ds.predictLetter(d, i)) {
                    echo(`'${k}'(${v}) `, { n: 1 });
                }
                echo(" ");
            }
...
function predictWord(ds, d) {
    for (let w of ds.predictWord(d, 1)) {
        echo("pw", w.pre, w.post);
    }
}
//# sourceMappingURL=test-ngram.js.map