...
export class Document {
    constructor(path, content, timeStamp = Date.now()) {
        this.path = path;
...
export class DocumentSet {
    ...
    predict(doc, offset) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield predict(doc, offset, this.root);
        });
    }
    ...
    //private
    LetterIndex() {
        let lidx = { count: 0, map: new Map(), keyType: "chr" };
        return lidx;
    }
    DocumentIndex(isEof, depth) {
        let didx = { count: 0, depth, map: new Map(), keyType: "doc", isEof };
        return didx;
    }
    addChr(map, doc, offset, depth) {
...
export function* resultsFrom(index, prefix = "") {
...
export function* find(word, rootidx) {
    let { index, found, rest } = findIndex(word, rootidx);
    if (keyIsLetter(index) && rest)
        return;
    if (!rest) {
        for (let { document, offset, prefix, eof } of resultsFrom(index)) {
            yield {
                document,
                offset: offset -
                    prefix.length -
        ...
        for (let { document, offset, prefix } of resultsFrom(index)) {
...
export function predict(doc, offset, rootidx) {
    return __awaiter(this, void 0, void 0, function* () {
        let p = 1;
        let c = new Counter();
        while (offset - p >= 0) {
...
export function* predictWord(doc, offset, rootidx) {
    let oo = offset;
    offset--;
    while (offset >= 0 && isAlpha(doc.at(offset)))
        offset--;
    offset++;
    if (!isAlpha(doc.at(offset)))
        return;
...
export function keyIsDocument(idx) {
    return idx.keyType == "doc";
}
export function keyIsLetter(idx) {
