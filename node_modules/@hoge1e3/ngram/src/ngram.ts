...
export class DocumentSet{
    ...
    DocumentIndex(isEof:boolean,depth:number):DocumentIndex{
        let didx:DocumentIndex={count:0, depth, map:new Map(), keyType: "doc",isEof};
        return didx;
    }
    addChr(map:Map<string,Index>,doc:Document, offset:number,depth:number){
        let ch=doc.at(offset);
        if(ch==null)return ;
        if(depth>MAX_INDEX_DEPTH) return;
        let idx=map.get(ch);
        if(!idx){
            const didx=this.DocumentIndex(ch===EOF,depth+1);
            map.set(ch,didx);
            //if(ch==EOF)didx.isEof=true;
            idx=didx;
        }
        this.add(idx,doc,offset+1,depth+1);
    }
    ...
    add(idx:Index,doc:Document,offset:number,depth:number=0){
        if(keyIsLetter(idx)){
            this.addChr(idx.map,doc,offset,depth);
    ...
    toLetterIndex(didx:DocumentIndex){
        ...
        if(didx.depth>=MAX_INDEX_DEPTH) return;
        let nmap=new Map<string,Index>();
        let c=0;
        for(let [doc, offsets] of didx.map){
            for (let o of offsets){
                this.addChr(nmap,doc,o,didx.depth);
                c++;
            }
        }
...
export function *resultsFrom(index:Index,prefix=""):Generator<ResultDetail> {
...
export function *find(word:string ,rootidx: LetterIndex):Generator<FindResult> {
...
export async function predict(doc:Document,offset:number,rootidx:LetterIndex){
    let p=1;
    let c=new Counter();
    while(offset-p>=0){
...
export function *predictWord(doc:Document,offset:number,rootidx:LetterIndex):Generator<PredictionResult>{
    let oo=offset;
    offset--;
...
export function keyIsDocument(idx:Index):idx is DocumentIndex{
    return idx.keyType=="doc";
}
export function keyIsLetter(idx:Index):idx is LetterIndex{
...
export interface LetterIndex {//Tree
    count:number;
    //isEof?: boolean;
    keyType: "chr";
    map:Map<string, Index>;
};
export interface DocumentIndex {//Leaf
    count:number;
    depth:number;
    isEof: boolean;
    keyType: "doc";
