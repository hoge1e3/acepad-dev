...
export const Sym_src=Symbol("src");
export const Sym_dst=Symbol("dst");
...
const sym_noderef=Symbol.for("sym_noderef");
const NODEREF_NAME="node-ref";
export function nodeRef(node:HTMLElement):HTMLElement {
    const res=document.createElement(NODEREF_NAME);
    (res as any)[sym_noderef]=node;
    return res;
}
export function unnodeRef<T extends Node>(node:T):T {
    const ref=(node as any)[sym_noderef]
    return ref ?? node;
}
export function update(_dst:HTMLElement|null,src:HTMLElement,opt:Options={}){
    let {
        findUpdatable,
        isUpdatable,
        isSkippable,
    ...
    const _isSkippable=isSkippable||((d:Node, s:Node)=>{
        return s===d || 
          isElement(s) && isElement(d) &&
            !!d.getAttribute(attr_version) &&
            d.getAttribute(attr_version)===
            s.getAttribute(attr_version);
    });
    const dst=_dst||document.querySelector("#"+src.getAttribute("id"))!;
    const updater={
        root:dst,
    ...
    function updateLink(d:any,s:any){
        if(d[Sym_src]){
          delete d[Sym_src][Sym_dst];
        }
      d[Sym_src]=s;
      s[Sym_dst]=d;
    }
    function u(d:Node, s:Node){
        ...
        if (d.nodeType===Node.TEXT_NODE) {
            ...
            return updateLink(d,s);
        }
        if (!isElement(s) || !isElement(d)) return updateLink(d,s);
        evm.overwrite(d,s);
        uattr(d, s);
        uprop(d, s);
        let i=0;
    ...
    for(let t of updater.trashed){
        ...
        if (!p) continue;
        if (!p.contains(t)) continue;
        if (p!==t.parentNode) continue;
        try {
            p.removeChild(t);
        } catch(e) {
          console.log("RMERR", p, t , p.contains(t),t.parentNode===p );
          console.log(e);
        }
    }
}