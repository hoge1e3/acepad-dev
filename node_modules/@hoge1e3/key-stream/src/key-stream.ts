...
const by={
  key:new Map<string,KeyItem>(),
  code:new Map<string,KeyItem>(),
  keyCode:new Map<number,KeyItem>(),
};
//
const k=[
...
for(let n of k){
  by.key.set(n.key, n);
  by.code.set(n.code, n);
  by.keyCode.set(n.keyCode, n);
}
...
*/
export type ModifierKeyName="Shift"|"Control"|"Alt"|"Meta";
const keyName2EventAttr={
  //key to event attr
  "Shift":"shiftKey",
  "Control":"ctrlKey",
  "Alt":"altKey",
  "Meta":"metaKey"
} as Record<ModifierKeyName, ModifierEventAttr>;
export type ModifierEventAttr="shiftKey"|"ctrlKey"|"altKey"|"metaKey";
const eventAttr2KeyName={
  "shiftKey":"Shift",
  "ctrlKey":"Control",
  "altKey":"Alt",
  "metaKey":"Meta",
} as Record<ModifierEventAttr, ModifierKeyName>;
...
export type ModifierState=Record<ModifierEventAttr, boolean|undefined>;
...
export function push({type,key,code,keyCode}:PushArg){
  let ko:KeyItem|undefined;
  if(key)ko=by.key.get(key);
  else if(keyCode)ko=by.keyCode.get(keyCode);
  else if(code)ko=by.code.get(code);
  if(!ko){
    ko={
      key:key!,//||code||String.fromCharCode(keyCode!),
      code:code!,//||key||String.fromCharCode(keyCode!),
      keyCode:keyCode!,//||(code||key!).charCodeAt(0),
    };
  }
  //if(!ko)throw new Error(`Invalid arg ${key} ${code} ${keyCode}`)
  const m=keyName2EventAttr[ko.key as ModifierKeyName];
...
export function setTarget(t:EventTarget){
  target=t;
}
export function sync(to:typeof modifierState){
  // emit events that make modifierState to `to`.
  for(let _k in to){
    const k=_k as ModifierEventAttr;
    if(modifierState[k] && !to[k]){
      push({type:"up",key:eventAttr2KeyName[k]});
    }else if(!modifierState[k] && to[k]){
      push({type:"down",key:eventAttr2KeyName[k]});
    }
  }
}
export function addEventListener(
  t:"keydown"|"keyup"|"keypress",
  h:(e:KeyboardEvent)=>boolean|undefined):void
/*export function addEventListener(
  ...a: Parameters<typeof target.addEventListener>
): void*/ {
  const ty=t.replace(/^key/,"");
  return target.addEventListener(ty,h as any);
}
...
/*export function addEventListener(
  ...
  const ty=t.replace(/^key/,"");
  return target.removeEventListener(ty,h as any);
}
