...
export function push({ type, key, code, keyCode }) {
    ...
    const m = keyName2EventAttr[ko.key];
    if (m)
        modifierState[m] = (type === "down");
...
export function addEventListener(t, h) {
    const ty = t.replace(/^key/, "");
    return target.addEventListener(ty, h);
}
export function removeEventListener(t, h) {
    const ty = t.replace(/^key/, "");
    return target.removeEventListener(ty, h);
}
...
//# sourceMappingURL=key-stream.js.map