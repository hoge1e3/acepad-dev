...
//const symdata=Symbol("tree-item");
const datamap=new WeakMap();
datamap.__id=Math.random();
  /**
  ...
  function render(self) {
    ...
    function renderLeaf(item) {
      let leaf,content,text,expando;
      if (item.rendered) {
          leaf=item.rendered.leaf;
          leaf.innerHTML="";
      } else{
          leaf = document.createElement('div');// content, children
          item.rendered={leaf};
          item.refresh=()=>renderLeaf(item);
      }
      content = document.createElement('div');//expando text
      text = document.createElement('div');
      expando = document.createElement('div');// + -
      leaf.setAttribute('class', 'tree-leaf');
      content.setAttribute('class', 'tree-leaf-content');
      content.setAttribute("id",Math.random()+"");
      datamap.set(content, item);
      // console.log("con", datamap.__id, content, datamap.get(content));
      ...
      expando.addEventListener("click",click);
      content.appendChild(expando);
      content.appendChild(text);
      leaf.appendChild(content);
      if (item.children && item.children.length > 0) {
        let children = document.createElement('div');
        item.rendered.children=children;
    ...
    function click(e) {
      ...
      const data = datamap.get(parent);//parent[symdata];//JSON.parse(parent.getAttribute('data-item'));
      //console.log("parent", datamap.__id, parent, data);
...
export function chkdatamap() {
    for (let d of document.querySelectorAll(".tree-leaf-content")) {
        console.log("cont2", datamap.__id, d, datamap.get(d));
    }
