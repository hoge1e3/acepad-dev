...
export const acted=Symbol("acted");
export const sym_editor=Symbol("editor");
export function activate1(base,element,opt){
    if(element[acted])return ;
    element[acted]=true;
    element.addEventListener("click",function (e){
      respondClick(base,this, opt, e);
    });
}
export function respondClick(base, element, opt, e) {
      console.log("overlay",element);
      const editor=overlay(element,{base,...opt});
      if(!e.clientX)return ;
...
export function activate(container,opt){
    if(opt.dynamic){
      ...
      container.addEventListener("click",(e)=>{
        const o=e.target;
        console.log("activate:t",o,o.tagName,o.type);
        if(o?.tagName) {
          const t=o?.tagName.toLowerCase();
          if ((t==="input" || t==="textarea") &&
            (!o.type || o.type==="text" || o.type==="textarea")){
            respondClick(container,o,opt,e);
          }
        }
      },{capture:true});
      return;/*
    ...
    for(let e of container.querySelectorAll("input")){
      if(e.type&&e.type!=="text")continue;
