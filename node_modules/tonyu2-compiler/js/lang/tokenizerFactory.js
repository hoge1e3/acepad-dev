import { StringParser } from "./parser.js";
...
export function tokenizerFactory({ reserved, caseInsensitive }) {
    ...
    function dtk(prev, name, parser, post) {
        if (name == SAMENAME) {
            if (typeof parser !== "string")
                throw new Error("dtk invalid arg");
            name = parser;
        }
    ...
    var num = tk(/^(?:0x[0-9a-f]+|0b[01]+|(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+)(?:e-?[0-9]+)?)/i, "'number'").ret(function (_n) {
        const n = _n;
        n.type = "number";
    ...
    var literal = tk({ exec: function (s) {
            ...
            return null;
        }, toString: function () { return "'literal'"; } }).first("\"'");
    var regex = tk({ exec: function (s) {
            ...
            return null;
        }, toString: function () { return "'regex'"; } }).first("/");
    const REG_DIV = (REG | DIV);
...
;
//# sourceMappingURL=tokenizerFactory.js.map