...
//-----------
export function genJS(klass, env, genOptions) {
    var srcFile = klass?.src?.tonyu; //file object  //S
    if (!srcFile) {
        console.log("No source file", klass);
        throw new Error("No souce file");
    }
    ...
    const buf = (genOptions.codeBuffer || env.codeBuffer ||
        new IndentBuffer({ fixLazyLength: 6, compress: env.options.compiler.compress }));
    ...
    const v = buf.visitor = new Visitor({
        ...
        jsonElem: function (node) {
            if (node.value) {
                ...
                buf.printf("%v: %f", node.key, function () {
                    ...
                    if (!si)
                        throw new Error("Scope info not set for " + node);
        ...
        "for": function (node) {
            ...
            if (node.inFor.type == "forin") {
                ...
                buf.printf("for (%s[%f] of %s(%v,%s)) {%{" +
                    "%f%n" +
                    "%}}", pre, loopVarsF(/*isVar,*/ inFor.vars), ITER2, inFor.set, inFor.vars.length, noSurroundCompoundF(node.loop));
            ...
            function loopVarsF(/*isVar: Token,*/ vars) {
                return function () {
                    vars.forEach((v, i) => {
                        var an = annotation(v);
                        if (i > 0)
                        ...
                        if (!si)
                            throw new Error("Scope info is not set for " + node);
    ...
    function genFiber(fiber) {
        ...
        var opt = true;
        if (!fiber.params)
            throw new Error("No parameter is set");
        //waitStmts=stmts;
    ...
    function genLocalsF(finfo) {
        ...
        function f() {
            ...
            if (!locals)
                throw new Error("No local info for" + finfo);
